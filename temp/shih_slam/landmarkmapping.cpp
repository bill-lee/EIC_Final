...
//robot(0,0,0) 在世界座標畫線
       ...
       pt1.x = cvRound(temp2.x + 1000*(-b)); //cvRound 把double變成int
...
void LandmarkMapping::DrawRobotPoseWithErrorEllipse(const RobotState robot, cv::Mat &img,bool showEllipse)
{
    ...
    if(showEllipse==true)
    {
        cv::Mat EigenValue(robot.robotPositionMean.rows,robot.robotPositionMean.cols,CV_64F),EigenVector(robot.robotPositionCovariance.rows,robot.robotPositionCovariance.cols,CV_64F);  //大小要可以變動 固定只能畫機器人橢圓
